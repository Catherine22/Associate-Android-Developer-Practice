...
public class PlaylistViewModelFactory implements ViewModelProvider.Factory {
    ...
    public static PlaylistViewModelFactory createFactory(Activity activity) {
        Application application = activity.getApplication();
        if (application == null) {
            throw new IllegalStateException("Not yet attached to Application");
        }
        return new PlaylistViewModelFactory(PlaylistRepository.getInstance(application));
    }

    private PlaylistViewModelFactory(PlaylistRepository repository) {
    ...
    public <T extends ViewModel> T create(@NonNull Class<T> modelClass) {
        try {
            return modelClass.getConstructor(PlaylistRepository.class).newInstance(mRepository);
